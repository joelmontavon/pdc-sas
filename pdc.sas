...
/*****************************************************************************************************************************
* This code converts these dates to days from epoch.
* SAS uses 1/1/1960 as the epoch.
* The %sysfunc() macro function allows use of data step functions outside of a data step.
* Note that character values (i.e., strings) do not need to be enclosed with quotation marks.
* Basically, we extract the portion of the date inside the quotes using the substr() function.
* Then, we convert the date to a number using the inputn() function.
* In the steps below, we will use these macro variables to set the indexes of our arrays to reflect the days in the measurement year.
*****************************************************************************************************************************/
...
/*****************************************************************************************************************************
* This code creates prescription claims test scenarios.
...
* Note that the drug column only includes the target medication. Combination products with multiple target drugs should have multiple rows for each fill--one per target medication.
...
/*****************************************************************************************************************************
* To calculate the PDC, we first identify the days covered by each medication.
...
* So, we need to adjust for overlapping days supply for fills of the same drug but not of different drugs.
* To accomplish this, we need two arrays.
* The drug level array will capture the days covered adjusting for overlapping days supply of the same drug.
* The patient level array will capture days covered across all of the patient's drugs.
* without adjusting for overlapping days supply for different drugs.
*****************************************************************************************************************************/
...
/*****************************************************************************************************************************
* Now we have everything we need to calculate the PDC.
